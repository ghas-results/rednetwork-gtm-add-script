___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "displayName": "Red Network - Carregamento do SDK",
  "description": "Faz o carregamento do script SDK da Red Network.",
  "categories": ["AFFILIATE_MARKETING", "CONVERSIONS", "ATTRIBUTION"],
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "brand": {
    "id": "brand_dummy",
    "displayName": "",
    "thumbnail": "data:image/png;base64,"
  },
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "TEXT",
    "name": "redNetworkId",
    "displayName": "Red Network ID",
    "simpleValueType": true,
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      },
      {
        "type": "STRING_LENGTH",
        "args": [
          9,
          9
        ],
        "errorMessage": "Insira corretamente o valor do RDN ID utilizando o seguinte formato: RDN-XXXXX"
      }
    ],
    "help": "Insira aqui o Red Network ID (RDN ID) que você recebeu na contratação do Red Network. O formato deve ser em texto sendo da seguinte maneira: \"RDN-XXXXXX\". Preencher este campo corretamente é crucial para a coleta dos dados. Em caso de dúvida, por favor, entrar em contato com o nosso suporte."
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const queryPermission = require('queryPermission');
const localStorage = require('localStorage');
if (localStorage) {
   localStorage.setItem('rednetwork_client_id', data.redNetworkId);
}

const injectScript = require('injectScript');
const url = 'https://rednetwork.redplatform.com.br/sdk/index.umd.js';
if (queryPermission('inject_script', url)) {
  injectScript(url, data.gtmOnSuccess(), data.gtmOnFailure());
}


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "inject_script",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://*.redplatform.com.br/*"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "access_local_storage",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "rednetwork_client_id"
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 15/06/2023, 15:29:53


